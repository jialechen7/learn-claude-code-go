...
func leadToolDefs() []*schema.ToolInfo {
	return []*schema.ToolInfo{
		...
		{Name: "shutdown_request", Desc: "Request a teammate to shut down gracefully. Returns request_id for tracking.", ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{"teammate": {Type: schema.String, Required: true}})},
		{Name: "shutdown_status", Desc: "Check status of a shutdown request by request_id.", ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{"request_id": {Type: schema.String, Required: true}})},
		{Name: "plan_approval", Desc: "Approve or reject a teammate's plan. Provide request_id + approve + optional feedback.", ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{"request_id": {Type: schema.String, Required: true}, "approve": {Type: schema.Boolean, Required: true}, "feedback": {Type: schema.String}})},
	}
}
